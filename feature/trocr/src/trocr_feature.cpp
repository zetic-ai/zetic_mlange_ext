...
ZeticMLangeTrocrFeature::ZeticMLangeTrocrFeature(YOLO_MODEL_TYPE yolo_model_type, const char* coco_file_path) {
...
ZeticMLangeTrocrFeature::~ZeticMLangeTrocrFeature() {
    delete(this->mlange_feature_opencv);
}

// TODO: We assign delete responsibility to user, possible hazard.
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::getByteArrayFromImage(cv::Mat &input_img,
                                                                              int8_t *blob) {
    Zetic_MLange_Feature_Result_t ret = ZETIC_MLANGE_FEATURE_FAIL;
...
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::preprocess(cv::Mat& input_img, cv::Mat& output_image) {
...
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::postprocess(std::vector<DL_RESULT>& output_dl_result,
                                                                    void* output)
{
    if (this->yolo_model_type == YOLO_CLS) {
...
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::resultToImg(cv::Mat& img, std::vector<DL_RESULT> res) {
    if (this->yolo_model_type == YOLO_CLS) { 
...
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::detectorResultToImg(cv::Mat& img, std::vector<DL_RESULT> res) {
...
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::classifierResultToImg(cv::Mat& img, std::vector<DL_RESULT> res) {
...
Zetic_MLange_Feature_Result_t ZeticMLangeTrocrFeature::readCocoYaml(const char* coco_file_path) {
