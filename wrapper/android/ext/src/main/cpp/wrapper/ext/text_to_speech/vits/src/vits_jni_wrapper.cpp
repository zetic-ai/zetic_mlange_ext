...
Java_com_zeticai_mlange_feature_texttospeech_vits_VitsWrapper_nativeInit(
  ...
  vits_tokenizer =
      new VitsTokenizer(convertJStringToCString(env, vocabulary_path));
}
...
Java_com_zeticai_mlange_feature_texttospeech_vits_VitsWrapper_nativeConvertTextToIds(
  ...
  auto [ids, attention_mask] =
      vits_tokenizer->convertTextToIds(ctext, maxLength);
  ...
  jmethodID pairCtor = env->GetMethodID(
      pairClass, "<init>", "(Ljava/lang/Object;Ljava/lang/Object;)V");
  jobject pairObject =
      env->NewObject(pairClass, pairCtor, jIdsArray, jMaskArray);
  return pairObject;
}